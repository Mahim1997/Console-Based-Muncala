...
public class MiniMax_PlayerMax {

    public int getBestMove(GameState state, int depth) {

        if (depth <= 0) {
            System.out.println("-->>In getBestMove() of MiniMax_PlayerMax ... depth given is <= 0 ... return -1");
            return -1;
        }
    ...
    private int min(GameState state, int depth) {
        ...
        if (depth == 0 /*|| game over */ || Helper.isGameOver(state)) {
            return heuristic_function(state);
        }
        if(state.isGameOver()){
            ///RETURNING NUM_STONES difference in STORAGE
